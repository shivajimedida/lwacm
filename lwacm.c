        ...
        for( x = 1; x < N_X; x++)
        {
          for( y = 1; y < N_Y; y++)
          {
            for( z = 1; z < N_Z; z++)
            {
                ...
                u[1][x][y][z][0] = ( f[1]-f[2]+f[7]-f[8]+f[9]-f[10]+f[11]-f[12]+f[13]-f[14] )/p_load;
                u[1][x][y][z][1] = ( f[3]-f[4]+f[7]+f[8]-f[9]-f[10]+f[15]-f[16]+f[17]-f[18] )/p_load;
                u[1][x][y][z][2] = ( f[5]-f[6]+f[11]+f[12]-f[13]-f[14]+f[15]+f[16]-f[17]-f[18] )/p_load;
            }
          }
        }
        
        // loop 2, store
        // we store the values in p[1][] into p[0][] after the calculation in a seperate loop
        // because some calculation still need p[0][] in the previous loop
        for( x = 1; x < N_X; x++)
        {
          for( y = 1; y < N_Y; y++)
          {
